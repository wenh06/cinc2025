...
"""
ST-MEM model implementation.

Adapted from:

- https://github.com/vuno/ST-MEM/blob/main/models/encoder/vit.py
- https://github.com/vuno/ST-MEM/blob/main/models/encoder/st_mem_vit.py
- https://github.com/vuno/ST-MEM/blob/main/models/st_mem.py
...
The input to the pretrain model is expected to be of shape ``(batch_size, num_leads, signal_length)``,
...
class ST_MEM(nn.Module, SizeMixin, CitationMixin):
    ...
    def __init__(
        ...
        self.decoder_pos_embed = nn.Parameter(torch.zeros(1, self.num_patches + 2, decoder_embed_dim), requires_grad=False)
...
def st_mem_vit_small_dec256d4b(encoder_only: bool = True, **kwargs):
    if encoder_only:
        model = ST_MEM_ViT(
            seq_len=2250, patch_size=75, num_leads=12, width=384, depth=12, mlp_dim=4 * 384, heads=6, qkv_bias=True
        )
    else:
        model = ST_MEM(
            embed_dim=384,
...
def st_mem_vit_base_dec256d4b(encoder_only: bool = True, **kwargs):
    if encoder_only:
        model = ST_MEM_ViT(
            seq_len=2250, patch_size=75, num_leads=12, width=768, depth=12, mlp_dim=4 * 768, heads=12, qkv_bias=True
        )
    else:
        model = ST_MEM(
            embed_dim=768,
