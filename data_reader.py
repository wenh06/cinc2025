        ...
        files.update(
            {
                ...
                "chagas_labels": f"https://moody-challenge.physionet.org/2025/data/{self.__chagas_label_file__}",
            }
        )
        return files
...
_SamiTrop_INFO = DataBaseInfo(
    name="Sami-Trop: 12-lead ECG traces with age and mortality annotations",
    about="""
    1. The whole Sami-Trop database consists of 12-lead ECG traces from 1959 patients **ALL** with chronic Chagas cardiomyopathy.
    2. The open-access dataset is a subset (1631 patients) of the Sami-Trop database, which contains annotations of age and mortality in addition to the 12-lead ECG traces.
...
class SamiTrop(_DataBase):
    ...
    def _ls_rec(self) -> None:
        ...
        self._df_records = pd.read_csv(self._label_file)
        self._df_records["sex"] = self._df_records["is_male"].map({True: "Male", False: "Female"})
        self._all_records = self._df_records.exam_id.unique().tolist()
